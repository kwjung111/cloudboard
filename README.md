...
- 비용 변동: KST 기준 T-2 비용을 동일 요일 중앙값과 직전 비용일에 비교한 요약과 상세 상승 항목
...
AWS Cost Explorer의 `NetAmortizedCost`를 통계 기준으로 분석합니다. 실행 시각과
무관하게 KST 오늘에서 이틀 전(T-2)을 기대 기준일로 사용하고, 직전 비용일과
최근 4주의 동일 요일 중앙값을 AI 근거로 제공합니다. T-2가 아직 없으면 그보다
이전의 최신 일자를 사용하고 `집계 지연`으로 표시합니다. 현재 청구 월의
`Estimated=true`는 일별 데이터 미완료 여부로 사용하지 않습니다.

비용 카드의 `상세`를 열면 Cost Explorer의 서비스·사용 유형별 기준일 비용,
동일 요일 중앙값, 상승액, 상승률을 확인할 수 있습니다. 비교 기준보다 증가한
항목은 개수 제한 없이 상승액이 큰 순서로 표시하며, 이전 동일 요일에 없던 항목은
`신규 비용`으로 구분합니다. 상세 목록은 화면을 처음 열 때 100개씩 불러오므로
항목이 많은 계정도 기본 대시보드 응답과 렌더링이 커지지 않습니다.
...
# 수동·중복 생성 요청의 최소 재조회 간격(초, 기본 300)
CLOUDBOARD_COST_REPORT_MIN_INTERVAL_SECONDS=300
```
...
- KST T-2 비용 기준일, 동일 요일 중앙값, 직전 비용일과 서비스별 비용 변동
...
Cost Explorer 조회 기간은 KST T-2를 마지막으로 포함하는 최근 42일입니다.
